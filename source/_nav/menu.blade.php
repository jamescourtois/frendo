    ...
    <button title="{{ $page->siteName }} Services" x-data="{subNavOpen: false}"
        ...
        Services
				<span class="block absolute right-0 top-0 w-[19px] h-[27px]">
					<span :class="subNavOpen ? 'rotate-[-45deg] bg-blue-600' : 'rotate-[45deg] bg-black'" class="block absolute transition duration-100 top-[50%] left-0 w-[12px] h-[3px] rounded-full group-hover:bg-blue-500"></span>
					<span :class="subNavOpen ? 'rotate-[45deg] bg-blue-600' : 'rotate-[-45deg] bg-black'" class="block absolute transition duration-100 top-[50%] right-0 w-[12px] h-[3px] rounded-full group-hover:bg-blue-500"></span>
				</span>
				<ul x-collapse x-show="subNavOpen" @click.outside="subNavOpen = false"  class="w-[250px] text-left absolute top-full left-0 duration-500 transition list-none m-0 p-0 bg-blue-100/90">
					<li>
						<a
